    ...
    while !remaining.is_empty() {
        ...
        while i < remaining.len() {
            ...
            match action {
                ...
                SeqAction::EndSeq => {
                    in_seq = false;
                    let strip_current = if config.has_detail_filters() {
...
impl FilterStream {
    ...
    pub const fn new() -> Self {
        Self {
            ...
            ss: crate::stream::StripStream::new(),
            strip_current: true,
        }
    }
    ...
    pub fn push(&mut self, input: &[u8], config: &FilterConfig, out: &mut Vec<u8>) {
        if config.is_strip_all() {
            // Delegate to StripStream — same base Parser as strip(),
            // one code path for StripAll regardless of entry point.
            self.ss.push(input, out);
            return;
        }
    ...
    pub fn push_write(
        ...
        if config.is_strip_all() {
            self.ss.push_write(input, writer)?;
            return Ok(());
        }
        for slice in self.filter_slices(input, config) {
                ...
                match action {
                    ...
                    SeqAction::InSeq => {
                        ...
                        if self.cp.current_kind() != SeqKind::Unknown {
                            *self.strip_current = self.config.should_strip(self.cp.current_kind());
                        }
                    }
                    SeqAction::StartSeq => {
