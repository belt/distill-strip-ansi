...
use crate::sgr_rewrite::rewrite_sgr_params;
                    ...
                    while i < self.remaining.len() {
                        ...
                        match action {
                            ...
                            SeqAction::EndSeq => {
                                ...
                                let needs_rewrite =
                                    self.stream.cp.current_kind() == SeqKind::CsiSgr
                                    && self.stream.cp.sgr_content() != SgrContent::empty();

                                if needs_rewrite {
                                    let rewritten = rewrite_sgr_params(
                                        &self.stream.seq_buf,
                                        self.config.depth,
                                    );
                                    self.stream.seq_buf.clear();
                                    return Some(TransformSlice::Owned(
                                        SmallVec::from_vec(rewritten),
                                    ));
                                }
