...
///
/// Lookup uses a tiered page index: `page_index[cp >> 8]` maps to
/// the range covering that 256-codepoint page, or `NO_RANGE` if
/// none. This makes lookup O(1) for the vast majority of codepoints
/// that aren't in any mapped Unicode block.
#[derive(Clone, Debug)]
pub struct UnicodeMap {
    ...
    /// Page index: `page_index[cp >> 8]` = range index, or `NO_RANGE`.
    /// Covers U+0000..U+FFFF (256 pages) + supplementary planes up to
    /// the highest mapped codepoint.
    page_index: Vec<u8>,
}

/// Sentinel: no range covers this page.
const NO_RANGE: u8 = 0xFF;
/// Sentinel: multiple ranges cover this page — fall back to linear scan.
const MULTI_RANGE: u8 = 0xFE;
...
impl UnicodeMap {
    ...
    pub fn from_sets(sets: Vec<CharMappingSet>) -> Self {
        ...
        let page_index = build_page_index(&all_ranges);
    ...
    pub fn lookup_char(&self, c: char) -> Option<char> {
        ...
        // Fast path: ASCII is never a homograph source.
        if cp < 0x80 {
            return None;
        }
        ...
        // Branchless page-index lookup. Page 0 (ASCII) is always
        // NO_RANGE in the index, so no special-case branch needed.
        // This avoids branch misprediction on CJK text where chars
        // alternate between ASCII punctuation and non-ASCII ideographs.
        if page < self.page_index.len() {
            ...
            // idx == NO_RANGE: no range covers this page.
        }

        // Binary search in pairs.
        self.all_pairs
    ...
    pub fn lookup_into(&self, c: char, out: &mut Vec<char>) -> bool {
        ...
        // Branchless page-index lookup — no ASCII special case.
        if page < self.page_index.len() {
            let idx = self.page_index[page];
            if idx < MULTI_RANGE {
                let r = &self.all_ranges[idx as usize];
    ...
    pub fn merge_set(&mut self, set: CharMappingSet) -> Result<(), String> {
        ...
        self.sets.push(set);
        // Rebuild page index to include the new ranges.
        self.page_index = build_page_index(&self.all_ranges);
        Ok(())
    }
...
/// Build a page index for O(1) range rejection.
///
/// `page_index[cp >> 8]` = range index if exactly one range covers
/// that 256-codepoint page, `MULTI_RANGE` if multiple, `NO_RANGE` if none.
fn build_page_index(ranges: &[RangeMapping]) -> Vec<u8> {
    // Always include at least page 0 (ASCII) so the lookup
    // never needs a separate ASCII branch.
    let max_cp = ranges.iter().map(|r| r.from_end).max().unwrap_or(0x100);
    let num_pages = ((max_cp >> 8) as usize + 1).max(1);
    let mut index = vec![NO_RANGE; num_pages];

    for (ri, r) in ranges.iter().enumerate() {
        if ri >= MULTI_RANGE as usize {
            break;
        }
        let first_page = (r.from_start >> 8) as usize;
        let last_page = (r.from_end >> 8) as usize;
        for entry in index
            .iter_mut()
            .take(last_page.min(num_pages - 1) + 1)
            .skip(first_page)
        {
            if *entry == NO_RANGE {
                *entry = ri as u8;
            } else if *entry != ri as u8 {
                *entry = MULTI_RANGE;
            }
        }
    }
...
mod toml_loader {
    ...
    fn parse_hex_cp(s: &str) -> Result<u32, String> {
        u32::from_str_radix(s.trim(), 16).map_err(|e| format!("invalid hex codepoint {s:?}: {e}"))
    }
    ...
    fn parse_hex_seq(s: &str) -> Result<Vec<char>, String> {
        s.split_whitespace().map(parse_hex_char).collect()
    }
    ...
    pub fn load_str(text: &str, source: String) -> Result<CharMappingSet, String> {
        let file: MappingFile =
            toml::from_str(text).map_err(|e| format!("failed to parse TOML from {source}: {e}"))?;
        ...
        for (i, r) in file.ranges.iter().enumerate() {
            ranges.push(convert_range(r).map_err(|e| format!("{source}: ranges[{i}]: {e}"))?);
        }

        let mut pairs = Vec::with_capacity(file.pairs.len());
        for (i, p) in file.pairs.iter().enumerate() {
            pairs.push(convert_pair(p).map_err(|e| format!("{source}: pairs[{i}]: {e}"))?);
        }
