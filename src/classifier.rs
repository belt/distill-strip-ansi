...
impl ClassifyingParser {
    ...
    #[inline]
    #[must_use]
    pub fn parser_state(&self) -> crate::parser::State {
        self.parser.state()
    }

    /// Returns `true` if the classifier has finished extracting
    /// metadata from the current sequence body and remaining body
    /// bytes can be skipped to the terminator.
    ///
    /// True when:
    /// - OSC: number is finalized (saw `;` or non-digit)
    /// - DCS: phase ≥ 3 (saw `$q` or determined not-query)
    /// - SOS/PM/APC/StringPassthrough: always (no metadata to extract)
    #[inline]
    #[must_use]
    pub fn body_done(&self) -> bool {
        match self.kind {
            SeqKind::Osc => (self.flags & FLAG_OSC_NUMBER_FINALIZED) != 0,
            SeqKind::Dcs => self.dcs_phase() >= 3,
            SeqKind::Sos | SeqKind::Pm | SeqKind::Apc => true,
            _ => false,
        }
    }
    ...
    pub fn feed(&mut self, byte: u8) -> SeqAction {
        ...
        // Fast path: ground content byte (~80%+ of input).
        // parser.feed already handled the Ground fast path;
        // if we were ground and got Emit, we're still ground.
        ...
        if !prev_ground && action == Action::Skip && now_ground {
            ...
            if (byte == 0x18 || byte == 0x1A) && !matches!(self.kind, SeqKind::Ss2 | SeqKind::Ss3) {
                self.sgr_content = SgrContent::empty();
                self.param_value = 0;
                self.param_state = ParamState::Inactive;
                self.first_param = 0;
            ...
            if self.param_state == ParamState::Normal && (self.flags & FLAG_SEEN_SEMICOLON) == 0 {
            ...
            if self.kind == SeqKind::Osc && (self.flags & FLAG_OSC_NUMBER_FINALIZED) == 0 {
    ...
    fn accumulate_param(&mut self, byte: u8) {
        use crate::classify_tables::ParamOp;

        // Activate on the first body byte of a CSI sequence.
        if self.param_state == ParamState::Inactive && self.kind == SeqKind::CsiOther {
        ...
        // OSC number accumulation — uses same digit/semicolon table.
        if (self.flags & FLAG_OSC_ACCUMULATING) != 0 {
            match crate::classify_tables::PARAM_OP[byte as usize] {
                ParamOp::Digit => {
                    let digit = u16::from(byte - b'0');
                    self.param_value = self.param_value.saturating_mul(10).saturating_add(digit);
                }
                _ => {
                    // Semicolon or non-digit: finalize OSC number.
                    self.finalize_osc_number();
                }
            }
            return;
        ...
        // DCS body inspection: detect DECRQSS ($q) via lookup table.
        if self.kind == SeqKind::Dcs {
            let phase = self.dcs_phase() as usize;
            if phase < 4 {
                let entry = crate::classify_tables::DCS_PHASE[phase][byte as usize];
                self.set_dcs_phase(entry & 0x7F);
                if entry & 0x80 != 0 {
        ...
        // CSI parameter accumulation — table-driven digit/semicolon dispatch.
        match crate::classify_tables::PARAM_OP[byte as usize] {
            ParamOp::Digit => {
                let digit = u16::from(byte - b'0');
                self.param_value = self.param_value.saturating_mul(10).saturating_add(digit);
    ...
    fn finalize_sgr_param(&mut self) {
        let v = self.param_value.min(255) as usize;
        let entry = crate::classify_tables::SGR_TABLE[self.param_state as usize][v];
        let new_state = entry >> 4;
        let bits = entry & 0x0F;
        // SAFETY: new_state is 0-5, matching ParamState repr(u8) variants.
        self.param_state = match new_state {
            0 => ParamState::Inactive,
            1 => ParamState::Normal,
            2 => ParamState::AwaitMode,
            3 => ParamState::Skip1,
            4 => ParamState::Skip2,
            5 => ParamState::Skip3,
            _ => ParamState::Normal,
        ...
        if bits != 0 {
            self.sgr_content |= SgrContent(bits);
        }
    }
    ...
    fn classify_if_introducer(&mut self, byte: u8) {
        ...
        self.kind = crate::classify_tables::INTRODUCER_KIND[byte as usize];
    }
    ...
    fn classify_csi_final(&mut self, byte: u8) {
        ...
        let k = crate::classify_tables::CSI_FINAL_KIND[byte as usize];
        let qp = crate::classify_tables::CSI_QUERY_PARAM[byte as usize];
        self.kind = if qp != 0 && self.first_param == qp {
            SeqKind::CsiQuery
        } else {
            k
        };
...
mod tests {
    ...
    fn sgr_mixed_basic_and_truecolor() {
        ...
        assert_eq!(p.sgr_content(), SgrContent::BASIC | SgrContent::TRUECOLOR);
    }
    ...
    fn sgr_mixed_basic_and_extended() {
        ...
        assert_eq!(p.sgr_content(), SgrContent::BASIC | SgrContent::EXTENDED);
    }
