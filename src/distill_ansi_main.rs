...
fn normalize_content(input: &[u8], map: &UnicodeMap) -> Vec<u8> {
    ...
    if modified { output } else { input.to_vec() }
}
...
const SHIPPED_FILES: &[(&str, &[&str])] = &[
    ...
    (
        "enclosed-cjk",
        &["cjk", "japanese", "korean", "canonicalize"],
    ),
    ("cjk-compatibility", &["japanese", "cjk", "canonicalize"]),
    (
        "halfwidth-katakana",
        &["japanese", "legacy-encoding", "canonicalize"],
    ),
    (
        "halfwidth-hangul",
        &["korean", "legacy-encoding", "canonicalize"],
    ),
    (
...
fn resolve_remove_builtins(spec: &str) -> Vec<&'static str> {
    match spec {
        "@security" => vec!["fullwidth_ascii", "math_latin_bold", "latin_ligatures"],
        "@ascii-normalize" => vec![
            "fullwidth_ascii",
            "math_latin_bold",
            "latin_ligatures",
