...
use crate::classifier::SeqDetail;
...
impl ThreatDb {
    ...
    pub fn from_file(path: &Path) -> Result<Self, String> {
        let text = std::fs::read_to_string(path)
            .map_err(|e| format!("failed to read threat-db: {e}"))?;

        let file: ThreatDbFile = toml::from_str(&text)
            .map_err(|e| format!("failed to parse threat-db TOML: {e}"))?;
    ...
    /// Linear scan — ≤20 entries, O(1) in practice.
    #[must_use]
    pub fn classify(&self, detail: &SeqDetail) -> Option<&ThreatEntry> {
        let kind_name = seq_kind_name(detail.kind);
        ...
        self.entries.iter().find(|entry| {
            ...
            // Optional field checks — if specified, must match.
