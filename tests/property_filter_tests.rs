...
use strip_ansi::{FilterConfig, SeqGroup, SeqKind, filter_strip, strip};
...
fn arb_csi_with_final(final_byte: u8) -> impl Strategy<Value = Vec<u8>> {
    (0u8..50, prop::collection::vec(0x30u8..=0x3F, 0..3)).prop_map(move |(code, params)| {
        let mut seq = vec![0x1B, b'['];
        seq.extend_from_slice(code.to_string().as_bytes());
        for p in params {
            seq.push(b';');
...
fn arb_mixed_ansi_input() -> impl Strategy<Value = Vec<u8>> {
    prop::collection::vec(
        prop_oneof![
            // Clean bytes (no ESC)
            prop::collection::vec(0x20u8..=0x7E, 1..16).prop_map(|v| v),
            // CSI SGR
...
fn osc_preserve_title_preserved_clipboard_stripped() {
    ...
    let config = FilterConfig::strip_all().no_strip_osc_type(OscType::Title);
...
fn seq_detail_accessor_snapshot() {
    ...
    assert!(
        !d.dcs_is_query,
        "dcs_is_query must be false for CSI SGR sequences"
    );
}
...
fn debug_idempotency_regression() {
    ...
    let input: &[u8] = &[
        103, 103, 114, 210, 11, 118, 139, 3, 123, 138, 198, 36, 190, 130, 52, 202, 198, 141, 135,
        90, 48, 190, 9, 106, 57, 50, 182, 23, 190, 162, 118, 102, 8, 54, 242, 128, 186, 227, 255,
        81, 87, 191, 211, 165, 142, 200, 85, 18, 205, 156, 49, 22, 47, 33, 184, 142, 78, 81, 157,
        226, 63, 63, 121, 208, 121, 205, 173, 75, 119, 222, 100, 188, 211, 69, 102, 23, 121, 20,
        187, 75, 133, 113, 185, 175, 35, 123, 43, 205, 210, 137, 38, 187, 234, 74, 179, 89, 123,
        224, 193, 55, 74, 133, 21, 184, 152, 198, 33, 217, 46, 146, 219, 125, 55, 135, 31, 205,
        175, 218, 223, 21, 126, 59, 92, 153, 121, 62, 241, 213, 114, 115, 43, 87, 106, 3, 41, 85,
        138, 230, 167, 254, 142, 147, 40, 67, 143, 112, 226, 204, 187, 158, 237, 246, 88, 243, 150,
        137, 56, 104, 217, 14, 81, 88, 220, 185, 78, 54, 153, 238, 55, 180, 165, 180, 76, 153, 33,
        242, 115, 86, 24, 45, 129, 134, 55, 21, 67, 230, 21, 197, 182, 94, 211, 128, 166, 204, 19,
        84, 172, 38, 61, 46,
    ];
...
fn debug_streaming_regression() {
    use strip_ansi::{FilterConfig, FilterStream, filter_strip};
    ...
    assert_eq!(
        &streaming, &*stateless,
        "streaming must equal stateless for strip_all"
    );
}
...
fn debug_p6_regression_exact() {
    ...
    let input: &[u8] = &[
        240, 94, 114, 144, 251, 36, 145, 37, 75, 39, 225, 191, 21, 128, 185, 202, 223, 130, 150,
        153, 147, 114, 2, 174, 17, 232, 242, 103, 79, 100, 124, 3, 110, 129, 169, 97, 186, 137,
        149, 201, 104, 118, 29, 64, 169, 92, 151, 231, 242, 252, 158, 130, 181, 122, 102, 127, 235,
        138, 89, 71, 178, 170, 114, 5, 68, 46, 52, 65, 223, 105, 103, 112, 250, 154, 2, 146, 9,
        212, 32, 175, 12, 245, 250, 199, 199, 10, 42, 242, 254, 6, 124, 167, 232, 102, 44, 182,
        180, 68, 165, 174, 218, 221, 243, 20, 61, 193, 255, 99, 15, 26, 157, 95, 183, 186, 16, 107,
        95, 95, 198, 90, 87, 89, 34, 6, 212, 117, 230, 173, 206, 122, 54, 46, 89, 134, 42, 95, 253,
        25, 107, 16, 146, 121, 23, 198, 80, 82, 148, 244, 213, 123, 4, 23, 184, 47, 157, 225, 18,
        238, 205, 46, 155, 231, 189, 6, 251, 78, 79, 159, 231, 227, 126, 66, 231, 51, 110, 79, 75,
        130, 88, 22, 91, 72, 103, 185, 55, 169, 170, 199, 91, 35, 49, 192, 32, 80, 213, 172, 251,
        93, 182, 179, 94, 46, 229, 217, 99, 131, 128, 34, 246, 123, 211, 50, 230, 105, 241, 141,
        191, 128, 120, 91, 43, 201, 182, 255, 56, 6, 200, 40, 85, 73, 128, 80, 76, 136, 133, 142,
        71, 238, 235, 239, 28, 131, 83, 2, 117, 231, 11, 125, 122, 97, 29, 103, 23, 21, 71, 73, 55,
        67, 175, 25, 214, 73, 19, 18, 8, 52, 15, 159, 80, 171, 217, 129, 0, 117, 52, 162, 33, 28,
        176, 57, 120, 197, 34, 64, 212, 77, 196, 89, 0, 186, 147, 227, 253, 62, 190, 87, 155, 121,
        198, 249,
    ];
