...
fn grey_index_to_value_formula() {
    for idx in 232..=255u8 {
        ...
        assert_eq!(
            grey_index_to_value(idx) as u16, expected,
...
fn truecolor_to_256_saturated_prefers_cube() {
    ...
    assert!(result >= 16 && result <= 231, "saturated color should map to cube");
}
...
fn rewrite_basic_colors_unchanged_at_256() {
    ...
    let input = sgr(b"31");
    let result = rewrite_sgr_params(&input, ColorDepth::Color256);
    assert_eq!(result, input, "basic color should be unchanged at 256 depth");
}
...
fn rewrite_to_mono_strips_basic_fg_bg() {
    ...
    for code in 30..=37 {
        assert!(!s.contains(&code.to_string()), "fg {code} should be stripped");
    }
    for code in 40..=47 {
        assert!(!s.contains(&code.to_string()), "bg {code} should be stripped");
    }
}
