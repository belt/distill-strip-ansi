...
use strip_ansi::{SeqKind, TerminalPreset, filter_strip};
...
fn from_name_standard_names() {
    assert_eq!(
        TerminalPreset::from_name("dumb"),
        Some(TerminalPreset::Dumb)
    );
    assert_eq!(
        TerminalPreset::from_name("color"),
        Some(TerminalPreset::Color)
    );
    assert_eq!(
        TerminalPreset::from_name("vt100"),
        Some(TerminalPreset::Vt100)
    );
    assert_eq!(
        TerminalPreset::from_name("tmux"),
        Some(TerminalPreset::Tmux)
    );
    assert_eq!(
        TerminalPreset::from_name("sanitize"),
        Some(TerminalPreset::Sanitize)
    );
    assert_eq!(
        TerminalPreset::from_name("xterm"),
        Some(TerminalPreset::Xterm)
    );
    assert_eq!(
        TerminalPreset::from_name("full"),
        Some(TerminalPreset::Full)
    );
}
...
fn from_name_aliases() {
    assert_eq!(
        TerminalPreset::from_name("pipe"),
        Some(TerminalPreset::Dumb)
    );
    assert_eq!(TerminalPreset::from_name("ci"), Some(TerminalPreset::Color));
    assert_eq!(
        TerminalPreset::from_name("pager"),
        Some(TerminalPreset::Color)
    );
    assert_eq!(
        TerminalPreset::from_name("screen"),
        Some(TerminalPreset::Tmux)
    );
    assert_eq!(
        TerminalPreset::from_name("safe"),
        Some(TerminalPreset::Sanitize)
    );
    assert_eq!(
        TerminalPreset::from_name("modern"),
        Some(TerminalPreset::Full)
    );
}
...
fn tmux_strips_dcs_and_osc_echoback() {
    let config = TerminalPreset::Tmux.to_filter_config();
    assert!(
        config.should_strip(SeqKind::Dcs),
        "tmux should strip DCS (DECRQSS)"
    );
    assert!(
        config.should_strip(SeqKind::Osc),
        "tmux should strip OSC (font query)"
    );
...
fn sanitize_strips_dangerous_csi_kinds() {
    let config = TerminalPreset::Sanitize.to_filter_config();
    assert!(
        config.should_strip(SeqKind::CsiQuery),
        "sanitize must strip CsiQuery"
    );
    assert!(
        config.should_strip(SeqKind::CsiDeviceStatus),
        "sanitize must strip CsiDeviceStatus"
    );
}
...
fn sanitize_strips_csi_21t_title_report() {
    ...
    assert!(
        result.is_empty(),
        "sanitize must strip CSI 21t (title report)"
    );
}
...
fn sanitize_strips_csi_6n_cursor_position() {
    ...
    assert!(
        result.is_empty(),
        "sanitize must strip CSI 6n (cursor position report)"
    );
}
...
fn sanitize_preserves_safe_osc_types() {
    ...
    assert_eq!(
        &*filter_strip(&title, &config),
        &title[..],
        "sanitize must preserve OSC Title"
    );

    // OSC 8 (Hyperlink) → preserved
    let hyperlink = osc(8, b";https://example.com");
    assert_eq!(
        &*filter_strip(&hyperlink, &config),
        &hyperlink[..],
        "sanitize must preserve OSC Hyperlink"
    );

    // OSC 9 (Notify) → preserved
    let notify = osc(9, b"notification");
    assert_eq!(
        &*filter_strip(&notify, &config),
        &notify[..],
        "sanitize must preserve OSC Notify"
    );

    // OSC 7 (WorkingDir) → preserved
    let workdir = osc(7, b"file:///home/user");
    assert_eq!(
        &*filter_strip(&workdir, &config),
        &workdir[..],
        "sanitize must preserve OSC WorkingDir"
    );

    // OSC 133 (ShellInteg) → preserved
    let shellinteg = osc(133, b"A");
    assert_eq!(
        &*filter_strip(&shellinteg, &config),
        &shellinteg[..],
        "sanitize must preserve OSC ShellInteg"
    );
}
