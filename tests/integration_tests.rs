...
fn head_limits_output_lines() {
    ...
    cmd.arg("--head")
        .arg("2")
        .write_stdin("\x1b[31mline1\x1b[0m\n\x1b[32mline2\x1b[0m\n\x1b[33mline3\x1b[0m\n");
...
#[test]
fn head_short_flag() {
    // -n is now line numbering (cat -n). --head is long-form only.
    let mut cmd = cmd();
    cmd.arg("--head").arg("1").write_stdin("aaa\nbbb\nccc\n");
...
fn max_size_caps_input() {
    ...
    cmd.arg("--max-size").arg("5").write_stdin("abcdefghij");
...
fn preset_dumb_matches_default_strip_all() {
    ...
    let default_output = cmd().write_stdin(input).output().unwrap();
    ...
    assert_eq!(String::from_utf8_lossy(&dumb_output.stdout), "red link\n",);
}
...
fn preset_color_preserves_sgr() {
    ...
    assert!(
        ...
        "color preset should preserve SGR"
    );
...
fn preset_xterm_with_unsafe_accepts() {
    ...
    assert!(
        stdout.contains("\x1b[31m"),
        "xterm --unsafe should preserve SGR"
    );
    assert!(
        stdout.contains("\x1b]8;"),
        "xterm --unsafe should preserve OSC hyperlinks"
    );
}
...
fn unsafe_with_safe_preset_is_noop() {
    ...
    assert!(
        stdout.contains("\x1b[31m"),
        "color --unsafe should still preserve SGR"
    );
}
...
fn unsafe_without_preset_is_ignored() {
    ...
    let output = cmd().arg("--unsafe").write_stdin(input).output().unwrap();
...
fn check_threats_detects_csi_21t_exit_77() {
    ...
    assert!(
        ...
        "fail mode should produce no stdout"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn check_threats_strip_mode_filters_and_reports() {
    ...
    assert!(
        !stdout.contains("\x1b[21t"),
        "threat sequence should be stripped"
    );

    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("csi_21t"),
        "should report csi_21t on stderr"
    );
}
...
fn on_threat_without_check_threats_is_error() {
    let mut cmd = cmd();
    cmd.arg("--on-threat=strip").write_stdin("test");

    let output = cmd.output().unwrap();
    assert_eq!(
        output.status.code(),
        Some(2),
        "clap should reject --on-threat without --check-threats"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn structured_threat_output_csi_21t_format() {
    ...
    let mut cmd = cmd();
    cmd.arg("--check-threats").write_stdin("\x1b[21t\n");
...
fn structured_threat_output_osc_50_format() {
    ...
    let mut cmd = cmd();
    cmd.arg("--check-threats").write_stdin("\x1b]50;?\x07\n");
...
fn structured_threat_output_dcs_decrqss_format() {
    ...
    let mut cmd = cmd();
    cmd.arg("--check-threats").write_stdin("\x1bP$q\"p\x1b\\\n");
...
fn structured_threat_output_csi_6n_no_cve() {
    // CSI 6n has no CVE
    let mut cmd = cmd();
    cmd.arg("--check-threats").write_stdin("\x1b[6n\n");
...
fn no_threat_report_requires_check_threats() {
    let mut cmd = cmd();
    cmd.arg("--no-threat-report").write_stdin("test");
...
"#,
    ...
    assert_eq!(
        output.status.code(),
        Some(77),
        "should exit 77 for custom threat"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
