...
mod with_toml {
    ...
    fn normalize_math_italic() {
        let text =
            std::fs::read_to_string("etc/unicode-mappings/math-latin.toml").unwrap();
        let set = load_str(&text, "test".into()).unwrap();
        let mut map = UnicodeMap::builtin();
        map.merge_set(set).unwrap();
        // 𝐴𝐵𝐶 (italic) → ABC
    ...
    fn normalize_cjk_compat_kiro() {
        let text =
            std::fs::read_to_string("etc/unicode-mappings/cjk-compatibility.toml").unwrap();
        let set = load_str(&text, "test".into()).unwrap();
        let mut map = UnicodeMap::builtin();
        map.merge_set(set).unwrap();
        // ㌔ → キロ
    ...
    fn normalize_arabic_beh() {
        let text =
            std::fs::read_to_string("etc/unicode-mappings/arabic-presentation-forms.toml")
                .unwrap();
        let set = load_str(&text, "test".into()).unwrap();
        let mut map = UnicodeMap::builtin();
        map.merge_set(set).unwrap();
