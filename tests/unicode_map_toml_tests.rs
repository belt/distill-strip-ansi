...
use strip_ansi::unicode_map::{Direction, UnicodeMap, load_str};
...
fn shipped_math_latin_parses() {
    ...
    assert!(
        !set.pairs.is_empty(),
        "should have Letterlike Symbols pairs"
    );
}
...
fn shipped_enclosed_parses() {
    let text = std::fs::read_to_string("etc/unicode-mappings/enclosed-alphanumerics.toml").unwrap();
...
fn shipped_hangul_parses() {
    let text = std::fs::read_to_string("etc/unicode-mappings/halfwidth-hangul.toml").unwrap();
...
fn shipped_cjk_compat_parses() {
    let text = std::fs::read_to_string("etc/unicode-mappings/cjk-compat-ideographs.toml").unwrap();
