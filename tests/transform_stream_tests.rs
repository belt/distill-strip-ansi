...
fn cross_chunk_sgr_stripped() {
    ...
    let result = transform_chunks(
        &[b"a\x1b[38;2;255", b";0;0mb"],
        ColorDepth::Color256,
    );
...
fn rewritten_sgr_yields_owned() {
    let config = TransformConfig::new(ColorDepth::Color256);
    let mut stream = TransformStream::new();
    let slices: Vec<TransformSlice> =
        stream.transform_slices(b"\x1b[38;2;255;0;0m", &config).collect();
    assert_eq!(slices.len(), 1);
...
fn non_color_sgr_yields_borrowed() {
    ...
    let mut stream = TransformStream::new();
    let slices: Vec<TransformSlice> =
        stream.transform_slices(b"\x1b[5A", &config).collect();
    assert_eq!(slices.len(), 1);
    assert!(
...
fn mixed_content_and_sgr() {
    ...
    let slices: Vec<TransformSlice> =
        stream.transform_slices(input, &config).collect();
    // Should have: "text" (borrowed), rewritten SGR (owned),
    // "red" (borrowed), reset (borrowed), "plain" (borrowed)
    assert!(slices.len() >= 3, "expected multiple slices, got {}", slices.len());
...
fn empty_input() {
    ...
    let mut stream = TransformStream::new();
    let slices: Vec<TransformSlice> =
        stream.transform_slices(b"", &config).collect();
