...
Uses `vt-push-parser` (a push-based VT-100 state machine).
Exports `strip_ansi_bytes(&[u8]) -> Cow<[u8]>` (same
zero-copy semantics as this crate), `strip_ansi_string`,
`StreamingStripper` (callback-based), and a `Writer` adapter.
No `no_std` support.

Good choice when: you need a VT-100 state machine and don't
need `no_std` or selective filtering.
