...
The library crate (`distill-strip-ansi`) uses
`#![forbid(unsafe_code)]` — zero unsafe in any code path
that touches user data.

The benchmark harness crate (`distill-bench-harness`) has
three functions that require `unsafe` for OS resource
queries. These are never compiled into the library or CLI
binaries — they exist only in bench binaries.

| Function           | Crate   | Why unsafe                 |
| ------------------ | ------- | -------------------------- |
| `current_rss_bytes`| harness | `libc::task_info` (macOS), |
|                    |         | `/proc/self/statm` (Linux) |
| `peak_rss_bytes`   | harness | `libc::getrusage` (Unix)   |
| `cpu_times_us`     | harness | `libc::getrusage` (Unix)   |

Why unavoidable: resident set size and CPU time are only
available through kernel syscalls. Rust has no safe stdlib
API for process-level memory or CPU accounting. The `libc`
crate provides the FFI bindings; the `unsafe` blocks call
`task_info` (macOS Mach API) and `getrusage` (POSIX) with
stack-allocated output buffers — no heap, no pointers to
user data, no mutation of shared state.

Each function is annotated with `#[allow(unsafe_code)]` at
function scope — the rest of the harness crate has no unsafe.
